...
#include "skeleton.h"
#include "joint.h"
#include "PROJ6850/matrix4x4.h"
#include "PROJ6850/vector4D.h"
...
void Skeleton::reachForTarget(map<Joint*, Vector3D> targets, double time) {
  ...
  //      Optimization should be performed by applying
      // gradient
      ...
      for (int i = 0; i < 100; i++) {
        ...
        for (map<Joint*, Vector3D>::iterator it = targets.begin(); it != targets.end(); it++) {
          Joint* goalJoint = it->first;
          Vector3D q = it->second;
          for (Joint* j = goalJoint; j != nullptr; j = j->parent) {
            j->calculateAngleGradient(goalJoint, q);
          }
        }

        for (Joint* j : joints) {
          Vector3D newAngle = j->getAngle(time) - tau * j->ikAngleGradient;
          j->setAngle(time, newAngle);
          j->rotation = newAngle;
        }
      }
...
Joint* PROJ6850::DynamicScene::Skeleton::loadJoint(XMLElement* jointNode,
                                                 set<double>& knotTimes) {

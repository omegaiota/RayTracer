...
XMLElement* ColladaParser::get_technique_PROJ6850(XMLElement* xml) {
  XMLElement* technique = get_element(xml, "extra/technique");
  while (technique) {
    string profile = technique->Attribute("profile");
    if (profile == "PROJ6850") return technique;
    technique = technique->NextSiblingElement("technique");
  }
...
void ColladaParser::parse_light(XMLElement* xml, LightInfo& light) {
  ...
  XMLElement* technique_PROJ6850 = get_technique_PROJ6850(xml);

  technique = technique_PROJ6850 ? technique_PROJ6850 : technique_common;
  if (!technique) {
...
void ColladaParser::parse_sphere(XMLElement* xml, SphereInfo& sphere) {
  ...
  XMLElement* e_technique = get_technique_PROJ6850(xml);
  if (!e_technique) {
...
void ColladaParser::parse_material(XMLElement* xml, MaterialInfo& material) {
  ...
  if (e_effect) {
    ...
    XMLElement* tech_PROJ6850 = get_technique_PROJ6850(e_effect);  // PROJ6850 profile

    if (tech_PROJ6850) {
      XMLElement* e_bsdf = tech_PROJ6850->FirstChildElement();
      while (e_bsdf) {
...
void ColladaWriter::writeHeader(ofstream& out) {
  ...
  out << "      <authoring_tool>CMU raytracer6850 (version "

...
// become invalid as soon as it is released.)
    ...
    float Vertex::laplacian() const {
      ...
      HalfedgeCIter h = halfedge();
      Vector3D v_i = h->vertex()->position;
      float sum = 0.f;
      do {
        Vector3D v_j = h->next()->vertex()->position, v_alpha = h->twin()->next()->next()->vertex()->position,
        v_beta = h->next()->next()->vertex()->position;

        //alpha
        Vector3D vec_Ai = v_i - v_alpha, vec_Aj = v_j - v_alpha;
        vec_Ai.normalize();
        vec_Aj.normalize();
        float cotAlpha = (float) (dot(vec_Ai, vec_Aj) / cross(vec_Ai, vec_Aj).norm());

        //beta
        Vector3D vec_Bi = v_i - v_beta, vec_Bj = v_j - v_beta;
        vec_Bi.normalize();
        vec_Bj.normalize();
        float cotBeta = (float) (dot(vec_Bi, vec_Bj) / cross(vec_Bi, vec_Bj).norm());

        float sum_cotan = cotAlpha + cotBeta;
        sum += sum_cotan * (h->next()->vertex()->offset - h->vertex()->offset);
        h = h->twin()->next();
      } while (h != halfedge());
      sum *= 0.5f;
      return sum;

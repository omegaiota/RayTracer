...
namespace PROJ6850 {
    ...
    Ray Camera::generate_ray(double x, double y) const {
      ...
      // convert coordinates in NDC [0,1] space to Screen space[-1,1]
      double_t screen_x = (2 * x - 1), screen_y = (2 * y - 1);

      // account for FOV
      double_t camera_x = screen_x * tan(radians(hFov) / 2), camera_y = screen_y * tan(radians(vFov) / 2);

      Vector3D d_camera = Vector3D(camera_x,camera_y ,-1); // technically should - o_camera but that's (0,0,0)
      Vector3D  d_world = c2w * d_camera;
      d_world.normalize();
      return Ray(position(), d_world);
    }
